...
export interface KebabTarget {
  id: string
  kind: 'project' | 'feature' | 'module'  // B2a: added 'module' — was TODO stub
...
interface Props {
  ...
  onClose: () => void
  onOpenItem: (id: string, kind: 'project' | 'feature' | 'module') => void
}
...
export function RoadmapKebabMenu({ target, onClose, onOpenItem }: Props) {
  const {
    archiveProjectV2, archiveFeatureV2, archiveModuleV2,
    updateProjectV2, updateFeatureV2, updateModuleV2,
    addProjectV2Decision, addFeatureV2Decision, addModuleV2Decision,
    permanentDeleteProjectV2, permanentDeleteFeatureV2, permanentDeleteModuleV2,
    projectsV2, featuresV2, modulesV2,
  ...
  const { id, kind, triggerRef, currentStatus } = target
  ...
  function doDecision() {
    ...
    run(async () => {
      ...
      else if (kind === 'feature') await addFeatureV2Decision(id, decisionText.trim())
      else await addModuleV2Decision(id, decisionText.trim())
    })
  }
  ...
  function doArchive() {
    run(async () => {
      ...
      else if (kind === 'feature') await archiveFeatureV2(id)
      else await archiveModuleV2(id, false)
    })
  }
  ...
  return (
    <Popover
      ...
      className="w-52 py-1"
    >
      <div role="menu" aria-label="Card options">
        <MenuItem icon={<Edit2 size={13} />} label="Edit" onClick={doEdit} />

        {/* Change status — not available for modules */}
        {kind !== 'module' && (
        ...
        {kind !== 'module' && <MenuItem icon={<Copy size={13} />} label="Duplicate" onClick={doDuplicate} disabled={busy} />}
